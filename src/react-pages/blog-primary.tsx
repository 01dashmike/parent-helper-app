...
export default function BlogPrimary() {
  ...
  const filteredPosts = posts.filter(post => 
    post.category?.includes('primary') && post.isPublished
  );

  if (isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-teal/10 via-white to-lavender/10">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal/10 via-white to-lavender/10">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <Link href="/blog">
            <Button variant="ghost" className="mb-4 text-teal hover:text-coral">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-sage max-w-2xl mx-auto">
              School readiness, early learning, and social development for primary age children.
            </p>
            <Badge variant="secondary" className="mt-4 bg-teal/10 text-teal">
        ...
        <div className="grid gap-8 md:gap-12">
          {filteredPosts.length === 0 ? (
            <div className="text-center py-12">
              <BookOpen className="w-16 h-16 text-teal/40 mx-auto mb-4" />
              <h3 className="text-xl font-semibold text-sage mb-2">Coming Soon</h3>
              <p className="text-sage/60">We're preparing valuable primary school readiness content for you.</p>
            </div>
          ) : (
